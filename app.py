...
with st.expander("\CID{1865} 数学的解析 (固有値とシステムの安定性)", expanded=True):
    ...
    if np.all(reals < 0):
        if np.any(np.abs(imags) > 1e-5):
            st.success(r"\UTF{2705} **安定な渦状収束 (Spiral Sink):** 全ての固有値の実部が負で、虚部が存在します。軍備は振動しながら均衡点へ収束します。")
        else:
            st.success(r"\UTF{2705} **安定な結節点 (Stable Node):** 全ての固有値が負の実数です。軍備は振動することなく滑らかに均衡点へ収束します。")
    elif np.any(reals > 0):
